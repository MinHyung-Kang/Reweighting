...
class MNISTImbalanced():
    def __init__(self, n_items = 5000, classes=[9, 4], weights=[0.9, 0.1], n_val_per_class=5, random_seed=1, mode="train", train_type="reweight"):
        ...
        if mode == "train":
            self.dataset = datasets.MNIST(DATA_DIR, train=True, download=True, transform=self.transform)
        else:
            self.dataset = datasets.MNIST(DATA_DIR, train=False, download=True, transform=self.transform)
            weights = [1.0] * len(classes)
            n_val_per_class = 0

        weights = np.divide(weights, np.sum(weights))

        n_class = list(map(int, np.floor(n_items * weights)))
        ...
        for ind, c in enumerate(classes):
            ...
            if train_type == 'reweight':
                ...
                if mode == 'train':
                    val_indices = np.concatenate((val_indices, tmp_idx[n_class[ind]:n_class[ind] + n_val_per_class]))    
            else:
                data_indices = np.concatenate((data_indices, tmp_idx[:n_class[ind] + n_val_per_class]))

                        
        # create loader for train/test data
        self.data_sampler = SubsetRandomSampler(data_indices.astype(int))

        print(f"Gathered {len(data_indices)} {mode} samples")
...
def get_mnist_loader(batch_size, classes=[9, 4], n_items=5000, weights=[0.9, 0.1], n_val_per_class=5, mode='train', train_type="reweight"):
    """Build and return data loader."""

    wrapper = MNISTImbalanced(classes=classes, n_items=n_items, weights=weights, n_val_per_class=n_val_per_class, mode=mode, train_type=train_type)
